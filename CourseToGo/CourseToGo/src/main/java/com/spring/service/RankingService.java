...
public class RankingService {
    ...
    public List<RankingItem> getTopRankingItems() {
        Aggregation aggregation = Aggregation.newAggregation(
        	    Aggregation.group("placeId").count().as("count")
                .first("placeId").as("placeId"), // Include the placeId field in the grouping result
            Aggregation.sort(Sort.Direction.DESC, "count"),
            Aggregation.limit(5)
        );

        AggregationResults<RankingItem> result = mongoTemplate.aggregate(
                aggregation, "chooseData", RankingItem.class
        );
