	...
	<script>
	    ...
	    document.getElementById("saveMark").addEventListener("submit", function(event) {
	        ...
	        // placeId에 저장된 값을 input 박스안에 각각 집어넣어졌는지 확인.
			  document.getElementById("placeId1").value = placeId[0] ? parseInt(placeId[0]) : null;
			  document.getElementById("placeId2").value = placeId[1] ? parseInt(placeId[1]) : null;
			  document.getElementById("placeId3").value = placeId[2] ? parseInt(placeId[2]) : null;
			  document.getElementById("placeId4").value = placeId[3] ? parseInt(placeId[3]) : null;
			  document.getElementById("placeId5").value = placeId[4] ? parseInt(placeId[4]) : null;
	        ...
	        if (placeId.length < 3) {
	            alert("최소 3개 이상의 플레이스 아이디를 선택해주세요.");
	            submitButtonClicked = false; // 버튼 클릭 상태 초기화
	            return;
	          }
